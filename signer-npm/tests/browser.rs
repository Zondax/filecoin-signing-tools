...
fn sign() {
    ...
    let answer_str =
        serde_json::to_string(&serde_wasm_bindgen::from_value::<MessageTxAPI>(answer).unwrap())
            .unwrap();

    let expected_answer_str = serde_json::to_string(
        &serde_wasm_bindgen::from_value::<MessageTxAPI>(expected_answer).unwrap(),
    )
    .unwrap();
