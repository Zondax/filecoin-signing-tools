...
pub fn sign_voucher(
    ...
    let svb = voucher.signing_bytes().map_err(|err| SignerError::GenericString(err.to_string()))?;
...
pub fn verify_voucher_signature(
    ...
    let sv_bytes = signed_voucher.signing_bytes().map_err(|err| SignerError::GenericString(err.to_string()))?;
