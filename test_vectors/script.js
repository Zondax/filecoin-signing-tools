...
identity.digest(Buffer.from('fil/6/multisig', 'utf-8'))
    .then(function (hash) {
        ...
        console.log('New base64 params: ' + Buffer.from(encoded_params_hex, 'hex').toString('base64'))
        console.log('New cbor value: ' + cbor_multisig_hex.replace(Buffer.from(cid.bytes).toString('hex'), Buffer.from(newCID.bytes).toString('hex')))
    })
